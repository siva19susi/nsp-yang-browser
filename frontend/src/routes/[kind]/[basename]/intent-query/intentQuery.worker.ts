...
import type { IntentQueryPostMessage } from "./structure"

onmessage = async (event: MessageEvent<IntentQueryPostMessage>) => {
  const payload = event.data

  postMessage({ type: "progress", value: 50 })
  try {
    const response = await fetch("/api/nsp/intent-explorer", {
